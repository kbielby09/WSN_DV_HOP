 ...
 */
    ...
    RoutingProtocol::NotifyInterfaceUp (uint32_t interface)
    {
      ...
      //socket->SetAcceptCallback(MakeNullCallback<bool, Ptr<Socket>, const Address &> (), MakeCallback(&RoutingProtocol::AcceptDvhop, this));
      std::cout << "Test NotifyInterfaceUp" << std::endl;
          ...
          if (!socket)
            {
              ...
              socket->SetRecvCallback (MakeCallback (&RoutingProtocol::RecvDvhop,this));
              //socket->SetAcceptCallback(MakeNullCallback<bool, Ptr<Socket>, const Address &> (), MakeCallback(&RoutingProtocol::AcceptDvhop, this));
              std::cout << "Test NotifyAddAddress" << std::endl;
              socket->BindToNetDevice (l3->GetNetDevice (interface));
          ...
          if (l3->GetNAddresses (interface))
            {
              ...
              socket->SetRecvCallback (MakeCallback (&RoutingProtocol::RecvDvhop, this));
              //socket->SetAcceptCallback(MakeNullCallback<bool, Ptr<Socket>, const Address &> (), MakeCallback(&RoutingProtocol::AcceptDvhop, this));
              //std::cout << "Test NotifyRemoveAddress" << std::endl;
              // Bind to any IP address so that broadcasts can be received
              socket->Bind (InetSocketAddress (Ipv4Address::GetAny (), DVHOP_PORT));
              socket->SetAllowBroadcast (true);
              m_socketAddresses.insert (std::make_pair (socket, iface));
   ...
   */
      ...
      for(std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator j = m_socketAddresses.begin(); j != m_socketAddresses.end (); ++j)
        {
          ...
          for (addr = knownBeacons.begin (); addr != knownBeacons.end (); ++addr)
            {
              ...
              hopCount = m_disTable.GetHopsTo (*addr); //LINE ADDED HERE
              
              //std::cout << "Test m_disTable.GetHopsTo(*addr): " << m_disTable.GetHopsTo (*addr) << std::endl;
              //std::cout << "Test sendHello: m_hopCount: " << helloHeader.GetHopCount() << std::endl;
          ...
          if (m_isBeacon){
              ...
              FloodingHeader helloHeader(m_xPosition,                 //X Position
                                         ...
                                         m_seqNo++,                   //Sequence Numbr
                                         hopCount,                     //EDIT MADE HERE Hop Count
                                         iface.GetLocal ());          //Beacon Address
              std::cout <<__FILE__<< __LINE__ << helloHeader << std::endl;

              std::cout << "Test: Reset: sendHello: m_hopCount: " << helloHeader.GetHopCount() << std::endl;
    ...
    {
      //std::cout << "Test RecvDvhop" << std::endl;
      ...
      //std::cout << "Test: RecvDvhop: m_hopCount: " << fHeader.GetHopCount() << std::endl;
    ...
    /**
     * Callback to accelpt callback
     */
    void RoutingProtocol::AcceptDvhop(Ptr<Socket> socket, const ns3::Address& from)
    {
